...
class PixelNorm(nn.Module):
    ...
    def forward(self, input_dict):
        input = input_dict['x']
        out_x = input * torch.rsqrt(torch.mean(input ** 2, dim=1, keepdim=True) + 1e-8)
        return {'x': out_x, 'tdl': input_dict['tdl']}
...
class EqualLinear(nn.Module):
    def __init__(
        self, in_dim, out_dim, bias=True, bias_init=0, lr_mul=1, activation=None, layerID=-1
    ):
        super().__init__()
...
class ModulatedConv2d(nn.Module):
    ...
    def forward(self, input, style):
        ...
        style = self.modulation({'x':style,'tdl':[]})['x'].view(batch, 1, in_channel, 1, 1)
...
class Generator(nn.Module):
    def __init__(
        ...
        for i in range(n_mlp):
            layers.append(
                EqualLinear(
                    style_dim, style_dim, lr_mul=lr_mlp, activation='fused_lrelu', layerID=i
                )
            )
    ...
    def mean_latent(self, n_latent):
        ...
        latent = self.style({'x':latent_in,'tdl':[]})['x'].mean(0, keepdim=True)

        return latent

    def get_latent(self, input):
        return self.style({'x':latent_in,'tdl':[]})['x']
